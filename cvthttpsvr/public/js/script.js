$(function() {
	...
	$("#testing_information").dialog({
		...
		open: function(event, ui) {
			...
			// runTest_1_0 RevitExtractor_x64_2015.0.2014.0519.zip
			var argstrarr =window.testjob_id.split(' ');
			var argstrarr1 = argstrarr[0];
			var filename = argstrarr[1];
			var argstrarr1arr = argstrarr1.split('_');
	...
	window.socket.on('test_information_hint', function(msg) {
		if(window.isTestingInformationPanelVisible){
			console.log(msg);
			var hintObj = localhip;//JSON.parse(msg);
